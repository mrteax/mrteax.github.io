(() => {
  ...
  if (feed && typeof MOMENTS !== 'undefined' && MOMENTS.length) {
    const liked = JSON.parse(localStorage.getItem('teax_liked') || '{}');

    MOMENTS.forEach((m, i) => {
      const key = m.time + '|' + (m.text || '').slice(0, 12);
      const isLiked = !!liked[key];
      const count = (m.likes || 0) + (isLiked ? 1 : 0);
      ...
      const meta = [];
      if (m.mood) meta.push(`<span class="m-mood">${m.mood}</span>`);
      if (m.place) meta.push(`<span class="m-place">📍 ${m.place}</span>`);
      const tags = (m.tags || []).map(t => `<span class="m-tag">#${t}</span>`).join('');
      const media = m.media ? `<div class="m-media" style="background:${m.media}"></div>` : '';
      const body = m.link
        ? `<a class="m-text" href="${m.link}">${m.text}</a>`
      ...
      item.innerHTML = `
        ...
        <div class="m-main">
          <div class="m-head">
            <span class="m-name">Tea X</span>
            <span class="m-time">· ${relTime(m.time)}</span>
          </div>
          ${body}
          ${media}
          <div class="m-meta">${meta.join('')}${tags}</div>
          <div class="m-actions">
            <button class="m-like${isLiked ? ' liked' : ''}" aria-label="点赞">
              <span class="m-heart">♥</span><span class="m-count">${count}</span>
            </button>
          </div>
        </div>`;

      const likeBtn = item.querySelector('.m-like');
      const countEl = item.querySelector('.m-count');
      likeBtn.addEventListener('click', () => {
        const store = JSON.parse(localStorage.getItem('teax_liked') || '{}');
        const now = !store[key];
        if (now) store[key] = 1; else delete store[key];
        localStorage.setItem('teax_liked', JSON.stringify(store));
        likeBtn.classList.toggle('liked', now);
        countEl.textContent = (m.likes || 0) + (now ? 1 : 0);
        if (now) {
    ...
    // Hero status badge mirrors the latest moment
    const heroStatus = document.getElementById('heroStatus');
    const latest = MOMENTS[0];
    if (heroStatus && latest && latest.mood) {
      heroStatus.innerHTML = `<i></i><span>${latest.mood}</span><em>· ${relTime(latest.time)}</em>`;
    }
  }
